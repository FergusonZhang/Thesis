...
if __name__ == '__main__':
    ...
    parser.add_argument(dest='chromosome', help='Please enter the chromosome number.', type=int)
    args = parser.parse_args()
    db = gffutils.create_db('Data_raw/Crubella_474_v1.1.gene.gff3', 'data.db', force=True, keep_order=True,
                            merge_strategy='merge', sort_attribute_values=True)

    infile = open(f'Data_extra/Cgrand_scaffold_{args.chromosome}_shapeit4.vcf_positions.pkl', 'rb')
    Positions = pickle.load(infile)
    infile.close()
    infile = open(f'Data_extra/Cgrand_scaffold_{args.chromosome}_shapeit4.vcf_scores.pkl', 'rb')
    ...
    if args.name == 'start':
        for mRNA in db.features_of_type('mRNA', order_by='start'):
            if mRNA.seqid == f'scaffold_{args.chromosome}':
    ...
    plt.savefig(f'Data_extra/Chromosome_{args.name}_{args.chromosome}')
